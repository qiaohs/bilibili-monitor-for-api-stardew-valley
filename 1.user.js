...
function check(element){
    ...
    $.get('https://space.bilibili.com/ajax/member/getSubmitVideos?mid=13046&page=1&pagesize=25&order=pubdate',function(data){
        ...
        else if(has==0&&status==0){$('.result').html('正在审核中！');status=1;$('.error-container').removeClass('g blink').addClass('blinkf');$('.m-loading span').html('监控中... 将在视频审核通过后发出通知！');}
        else if(has==0&&status==1){numup1($('.m-loading span'),"\u5ba1\u6838\u541b\u53c8\u7761\u7740\u4e86\uff01\u007a\u007a\u005a");}
        else if(has==1&&status==1){$('.result').html('已更新！');status=2;$('.error-container').removeClass('blinkf blink').addClass('g');$('.m-loading').html('监控完成，还不快去看视频？');}
